...
\begin{problem}
	...
	\begin{subproblem}
The modified \textit{get\_rewards} function is seen in Listing \ref{reward values}.
\begin{lstlisting}[language=Python, caption=Reward values, label=reward values] 
...
Invalid moves and losing a game are assigned reward values of -0.5 and -1 respectively. Making an invalid move is unwanted since it effectively skips the player's turn. An efficient algorithm should not make invalid moves. Losing a game is clearly unwanted, and thus its reward is the most negative of all actions.		
	\end{subproblem}
